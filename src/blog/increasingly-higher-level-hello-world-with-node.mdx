...
![Screenshot of some code reading `socket.end('Hello world!')l`](./images/hLi3AHU21.webp)
...
This is about as low-level as we can go here seeing as the web runs on [TCP](https://en.wikipedia.org/wiki/Transmission_Control_Protocol). Everything else will be built upon this layer. There is no cruft here: no HTML, no CSS, not even headers will be sent.
...
[HTTP](https://en.wikipedia.org/wiki/HTTP) is built upon TCP. It defines a communication structure for displaying web pages. Headers are now sent along with content.
...
Moving up, let's try it with [Express](https://expressjs.com/) now. Express is built on top of Connect, so it provides the same functionality, plus more robust features such as dynamic views and environment-based configurations.
...
Finally, we have [Zappa](https://zappajs.org/). This platform extends Express, providing support for CoffeeScript and Socket.IO. If you've never used CoffeeScript, it's an optimized language that directly compiles down to plain JavaScript.

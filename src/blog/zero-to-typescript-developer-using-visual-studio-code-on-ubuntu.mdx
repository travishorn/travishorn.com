...
_Note: The remaining part of this post uses the built-in debugger in VS Code. This is really only good for back-end coding. The debugger runs Node behind the scenes. In the future, I may do some research into getting the debugger to run a headless browser, which would be much more useful for front-end coding._

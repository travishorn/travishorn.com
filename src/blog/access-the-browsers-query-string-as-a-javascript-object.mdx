...
If it does already exist, that means there is already a value assigned to it. In this case, we need a way to show the current value(s) _and_ the new value. An array is a great option here. If the current value isn't already an array, we need to _make_ it an array.

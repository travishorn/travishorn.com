...
web — even if they don't *appear* to be — are always rectangular.
...
![The same circle, but with a mouse cursor nearby. The cursor is in a place that
*would be* clickable as a hyperlink, if the hyperlink was completely square
around the circle, but since the hyperlink hugs the circle, the mouse cursor
stays in its default form, indicating we are not hovering over a
link](./images/pfkPwIyAW.webp)
...
![A shape, filled in blue, reminiscent of a classic bookmark with two "tails".
The text "I'm a link" appears in white inside the
shape](./images/cvkJJDBC4.webp)

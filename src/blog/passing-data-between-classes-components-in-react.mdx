...
datePublished: 2016-12-06 23:01:01+00:00
tags:
  - html
  - javascript
  - react
...
We give `NameInput` a `name` attribute and an `onChange` attribute. `name` is set to the state's `name` property, while `onChange` is set to the `changeName()` function. Now, our text input will know what to do _on change_.
...
```jsx
ReactDOM.render(<App name="Travis" />, document.getElementById("app"));
```

...
![An empty text input. A submit button immediately follows. An alert popup appears from the text input saying, "Please fill out this field."](./images/Qg4LJSZrf.webp)
...
`wrongNumber` is a _little_ more complex, but not bad at all. Return `true` if the value is not a number **or** it's less than 1 **or** it's greater than 10.
...
Now the form will not submit if there are validation errors. But, at this point, the user doesn't know exactly _why_ the form didn't submit.
...
The `is-invalid` class will only be applied when both `attemptSubmit` is `true` _and_ when `missingName` (or `wrongNumber`) is `true`.

That's it! On load, the form looks like…

![Two empty text inputs and a button. The first text input is labeled "Name". The second is labeled "Enter a number between 1 and 10". The button is labeled "Submit".](./images/hiZta1zMW.webp)

If you try to submit with an empty name, a warning appears.

![The same UI, but the value "5" has been entered in the second input. The first input is outlined in red and has red text underneath that reads "The field is required".](./images/hLPFoNw7z.webp)

Or similar if you try to submit a number outside the range.

![The same UI again, but the first input contains the value "Travis" and the second input contains the value "15". The second input is outlined in red and has red text underneath that reads, "Make sure this is a number between 1 and 10."](./images/i3SBZjjK2.webp)

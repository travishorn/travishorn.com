...
fonts](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/Text_styling/Web_fonts).
...
**Roboto Mono**, include this `<link>` tag in the `<head>` of your page:
...
property](https://vuejs.org/guide/essentials/computed) that takes the raw
standings array, and enhances it.
...
[`.orderBy()`](https://lodash.com/docs/#orderBy) function that accepts an
...
scale](https://d3js.org/d3-scale/sequential) that uses the [RdYlGr (red, yellow,
green) color
scheme](https://d3js.org/d3-scale-chromatic/diverging#interpolateRdYlGn). It
then uses [`D3.extent()`](https://d3js.org/d3-array/summarize#extent) to get the
mininum and maximum `pct` values and sets that as the [scale's
domain](https://d3js.org/d3-scale/linear#linear_domain). Finally, it returns the
color based on the input number.

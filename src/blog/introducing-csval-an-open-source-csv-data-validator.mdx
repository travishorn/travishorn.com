...
datePublished: 2019-10-10 22:01:01+00:00
tags:
  - data-science
  - git
  - javascript
...
csval is a command-line tool and a JavaScript library that can check CSV files
against a set of validation rules. Some of the rules you can check for include…
...
csval can be installed and used as a CLI. It does, however, require
[Node.js](https://nodejs.org/en). If you have Node.js installed, you can install
csval from the command line.
...
Once installed, it can be called anywhere using the `csval` command. Give the
path to a CSV file as the first argument.
...
validation. The CLI will show errors if they exist. Otherwise, it will display a
success message.
...
Note that the line `"type": "object"` above is implied and can be left out if
desired. I only included it here to demonstrate that this file should follow the
JSON Schema format.
...
The code above uses inline CSV data and rules. You can also read CSV data and
rules from files.
...
csval is open source and I welcome all pull requests. The code is hosted on
GitHub: [travishorn/csval](https://github.com/travishorn/csval).
...
csval has 100% test coverage at the moment. Tests are run via
...
csval is written according to [Prettier code format](https://prettier.io/). You
can lint all files for errors.
...
csval is licensed under the MIT license. I welcome all pull requests and
contributions at the repository on GitHub:
[travishorn/csval](https://github.com/travishorn/csval).

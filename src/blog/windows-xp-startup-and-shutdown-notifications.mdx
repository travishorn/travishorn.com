...
This step-by-step guide will show you how to create startup and shutdown scripts
that will notify another PC when the target PC is turned on or off.

### Create the Scripts

The first step is to create the scripts that will run when the computer is
turned on or off.
...
1. Make sure to change [IP Address] to the IP address of the machine receiving
   the messages.
...
1. Click **Browse…**, find the file **C:\Scripts\startup.bat** and click
   **Open**
...
1. Click **Browse**, find the file **C:\Scripts\shutdown.bat** and click
   **Open**
...
Finally, to ensure the messages go through, the Messenger service must be
running.
...
Cover photo by [Tim Gouw](https://unsplash.com/@punttim).
